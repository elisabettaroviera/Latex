...
\begin{minipage}{0.68\textwidth}
    ...
    {\large d-- Data Pre-processing Pipeline \\ Comparison with \textit{An empirically driven data reduction
...
The analysis evaluates cross-tissue overlap, selection stability, and baseline methylation and variability properties of the resulting CpG sets. The proposed framework defines the final structural step of the data pre-processing pipeline.
...
\section{Overview of \textit{An Empirically Driven Data Reduction Method on the Human 450K Methylation Array to Remove Tissue-Specific Non-Variable CpGs} \cite{Edgar2017}}
...
While Step A evaluates robustness under a fixed non-variability definition, Step B analyses how selection stability and list size jointly depend on the variability threshold.

Figure~\ref{fig:stepB_stability_vs_size} shows the mean Jaccard similarity across LODO folds together with the mean number of CpGs classified as non-variable as a function of the threshold. Increasing the threshold leads to larger CpG sets and higher stability; however, the gain in stability rapidly saturates, whereas the list size continues to increase almost linearly.

The threshold proposed by \cite{Edgar2017} ($\mathrm{thr}=0.05$) lies at a clear inflection point of this relationship. At this value, stability is already high and consistent across folds, while further increases in the threshold yield only marginal stability improvements at the cost of a substantial expansion of the CpG set. This identifies $\mathrm{thr}=0.05$ as an empirically optimal compromise between reproducibility and dimensionality reduction in the breast tissue setting, providing a data-driven justification for retaining the original threshold within the extended framework.
...
\begin{subfigure}[t]{0.48\textwidth}
  ...
  \caption{Step A: stability of non-variable CpG selection under a LODO strategy.}
...
This observation is consistent with the original findings reported in~\cite{Edgar2017}, who noted that non-variable CpGs tend to localize at extreme methylation states. Importantly, the same pattern is preserved when breast tissue is included, providing additional support for the interpretation of non-variability as a genuine empirical property of the methylation landscape rather than an artifact of tissue composition or disease-related heterogeneity.
...
This extended analysis represents the final conceptual step of the preprocessing pipeline, immediately preceding dataset-specific feature selection. Its role is to define and validate a principled criterion for identifying non-variable CpGs, grounded in empirical stability and biological interpretability.

In the current study, datasets are analysed independently, and the non-variability threshold is therefore estimated separately within each cohort rather than enforcing a single shared CpG set across datasets. The extended framework provides a common rationale and validation strategy for this filtering step, while preserving dataset-specific methylation structure and avoiding premature cross-cohort harmonisation.
