...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[height=6cm]{Figures/type_bias_density.png}
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[height=6cm]{Figures/type_bias_qqplot_with_legend.png}
...
\good{Accordingly, the reference threshold proposed by Edgar~\textit{et~al.}~\cite{Edgar2017}, $r_{\beta} < 0.05$, was adopted as a pragmatic cutoff and applied to the Normal--Adjacent subset, leaving a final matrix of \textbf{99 samples $\times$ 156,740 CpGs}.}  
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Figures/fig2_norm_vs_adj_row.png}
  \caption{\textbf{Histograms} of $\beta$ (left pair) and $M$ (right pair) for Normal (labels 0,3) vs Adjacent (label 1), using all samples and CpGs after filtering.
  The bounded, U-shaped $\beta$ distributions contrast with the more symmetric $M$ distributions, visually supporting Du \textit{et al.}~\cite{Du2010}.}
  \label{fig:hist_norm_adj_row}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Figures/fig3_norm_vs_adj_row.png}
  \caption{\textbf{Mean--SD relationship} across CpGs for Normal vs Adjacent in $\beta$ (left pair) and $M$ (right pair).
  The local-median SD curves show pronounced mean-dependent variance in $\beta$ that is substantially reduced in $M$, consistent with the homoscedasticity advantage reported by Du \textit{et al.}~\cite{Du2010}.}
  \label{fig:meansd_norm_adj_row}
\end{figure}

\paragraph{Rationale.}
As shown by Du \textit{et al.}~\cite{Du2010}, $\beta$ is \emph{bounded} and \emph{heteroscedastic} (variance depends on the mean, especially near 0 and 1), whereas $M$ is approximately \emph{homoscedastic} and more suitable for linear modeling and $t$-tests.  
\emph{Interpretability} remains higher for $\beta$, hence I use $M$ for inference and $\beta$ for reporting.

\paragraph{Implementation in this dataset.}
The GSE69914 matrix provides $\beta$ only; I therefore computed $M=\log_2\!\big((\beta+\varepsilon)/(1-\beta+\varepsilon)\big)$ with a small $\varepsilon = 10^{-6}$ for numerical stability.  
\good{The working matrix was converted to $M$-values.}
