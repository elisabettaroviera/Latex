...
QUA VORREI FAR VEDERE CHE QUESTA COSA è STATA FATTA -> MI BASTA QUINDI UN PLOT PER DIMOSTRARE CHE NEI RAW DATA è STATA FATTA 
...
Each mask flags probes to be removed due to specific technical artifacts:
\begin{itemize}[label=-]
  \item \texttt{MASK\_mapping}: cross-hybridisation or ambiguous alignment;
  \item \texttt{MASK\_snp5}: SNP within $\pm 5$\,bp of the CpG;
  \item \texttt{MASK\_extBase}: SNP at the single-base extension;
  \item \texttt{MASK\_commonSNPs}: overlap with common polymorphisms;
  \item \texttt{MASK\_nonCG}: non-CpG-targeting probes;
  \item \texttt{MASK\_chrXY}: probes located on X or Y chromosomes (optional).
\end{itemize}
...
For the MethylationEPIC array, McCartney \emph{et al.} \cite{mccartney2016} assessed probe design artifacts and published supplementary tables that include:
\begin{itemize}[label=-]
...
\toprule
\textbf{Category} & \textbf{Naeem14} & \textbf{Chen13} & \textbf{Pidsley16} & \textbf{Zhou16} & \textbf{McCartney16} \\
\midrule
...
Sex-chromosome probes (X/Y)   	& Yes & Yes & Yes & \texttt{MASK\_chrXY} & -- \\
\bottomrule
\end{tabular}
} 
\end{table}
