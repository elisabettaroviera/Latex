...
The initial pre-processing stage verified the structural integrity of the dataset.  
...
A diagnostic evaluation of Infinium probe–design bias was conducted to assess the relative scaling of Type~I and Type~II probes.  
All probes were stratified by Infinium design type (Type~I vs.\ Type~II) using the official Illumina EPIC v1.0 manifest, obtained from the corresponding Bioconductor 
annotation package \cite{Illumina450kAnno}.  
This yielded \good{119{,}760 Type~I} and \good{627{,}842 Type~II} probes.
...
Technical probe filtering was applied to exclude CpG loci affected by unreliable hybridisation, 
SNP interference, multi-mapping, or annotation inconsistencies, using the same curated resources 
described in Appendix~\ref{app:probe-filters}.  
...
\begin{itemize}[label=-]
    \item \good{101{,}014} CpGs were flagged by Naeem et al.\ \cite{naeem2014};
    \item \good{11{,}759} by the Pidsley/Chen blacklist \cite{pidsley2016,chen2013};
    \item \good{114{,}407} by the Zhou \texttt{MASK\_*} reliability annotations \cite{zhou2016};
    \item \good{0} by the Chen cross-reactive probe list (no overlapping CpGs in this dataset);
    \item \good{1{,}585} by the McCartney cross-hybridisation resource \cite{mccartney2016}.
\end{itemize}
...
targeting non-CpG contexts (IDs beginning with ``\texttt{ch}'') were removed.  
This step eliminated \good{773 non-CpG loci}, reducing the working set to \good{544{,}754 CpGs}.

\subparagraph{Removal of sex-chromosome probes (X/Y)}
Probes mapped to chromosomes~X and~Y were excluded to avoid sex-related 
confounding effects and to maintain autosomal consistency.  
...
\texttt{removed\_cpgs\_all\_filters\_summary.csv}, including the contributing technical lists 
and the \texttt{Source\_Count} field indicating redundancy across filtering resources.
...
substantial. These effects are illustrated in Fig.~\ref{fig:gse287331_typeI_II_post}.
