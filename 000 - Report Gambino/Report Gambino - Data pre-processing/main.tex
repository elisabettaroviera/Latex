...
Abbiamo acquisito il dataset \texttt{GSE69914}~\cite{GSE69914} in formato \texttt{.txt} contenente i valori di metilazione ($\beta$-values) e ottenuto i label direttamente da \texttt{GEOparse} per garantirne la massima correttezza.  
...
Questa pipeline include la \textbf{correzione del bias tra le sonde Infinium di tipo~I e tipo~II}, assicurando che le distribuzioni dei valori di metilazione dei due design risultino comparabili prima delle analisi successive.

Abbiamo verificato in modo indipendente che tale correzione fosse effettivamente applicata.  
A tal fine, le distribuzioni dei $\beta$-values sono state stratificate per tipo di sonda (Type~I vs.~Type~II) utilizzando il manifesto ufficiale di Illumina~450K ottenuto dal pacchetto Bioconductor~\cite{Illumina450kAnno}.  
...
Il filtraggio tecnico ha lo scopo di rimuovere le sonde inaffidabili o affette da artefatti biologici o di piattaforma prima delle fasi di normalizzazione e modellazione.  
...
Considerando tutti i tipi di tessuto, la distribuzione di $r_{\beta}$ risulta fortemente asimmetrica, con la maggior parte dei loci che mostrano una variabilità limitata (\autoref{fig:beta_range_all}).  
...
Poiché l’obiettivo finale è identificare le CpG informative per distinguere i tessuti \textbf{normali} da quelli \textbf{adiacenti}, abbiamo applicato il filtraggio in modo specifico a questo sottoinsieme.  
...
\good{Abbiamo pertanto adottato come soglia di riferimento il valore proposto da Edgar~\textit{et~al.}~\cite{Edgar2017}, $r_{\beta} < 0.05$, applicandolo al sottoinsieme Normal--Adjacent, ottenendo una matrice finale di \textbf{99 campioni $\times$ 156.738 CpG}.}  
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        ...
        \caption{Distribuzione dell’intervallo inter-decile $r_{\beta}$ considerando l’intero dataset (Normal, Adjacent e Tumor).  
...
{\color{red}\textbf{DOMANDA:} il filtro $r_{\beta}<0.05$ è stato applicato solo al sottoinsieme Normal–Adjacent.  
È metodologicamente corretto, o sarebbe meglio rimuovere le CpG utilizzando il filtro sull’intero dataset?}

\section{Confronto tra quantificazioni in Beta-value e M-value}\label{sec:beta_mvalue_comparison}

La piattaforma Illumina Infinium fornisce due segnali corretti per ogni CpG: metilato ($y^{(M)}$) e non metilato ($y^{(U)}$).  
Il \textbf{Beta-value} rappresenta la frazione di metilazione, mentre l'\textbf{M-value} è la sua trasformazione logaritmica:
\[
\beta_i = \frac{\max(y_i^{(M)},0)}{\max(y_i^{(M)},0)+\max(y_i^{(U)},0)+\alpha}, \qquad
M_i = \log_2\!\left(\frac{\beta_i}{1-\beta_i}\right).
\]

\paragraph{Applicazione al dataset.}
Per la matrice GSE69914 gli $M$-value sono stati calcolati come
\[
M = \log_2\!\left(\frac{\beta+\varepsilon}{1-\beta+\varepsilon}\right),
\]
con $\varepsilon = 10^{-6}$ per stabilità numerica.  
\good{Il dataset è stato convertito in M-value per le successive analisi.}


Come mostrato da Du \textit{et al.}~\cite{Du2010}, i valori $\beta$ sono \emph{limitati} e \emph{eteroschedastici}, mentre gli $M$ risultano più simmetrici e \emph{omoschedastici}, quindi più adatti a modelli lineari.  
Come mostrato in \autoref{fig:hist_msd}, le distribuzioni dei $\beta$ assumono la tipica forma a U e mostrano varianza dipendente dalla media, mentre gli $M$ presentano profili più regolari.  
\good{I risultati ottenuti concordano con quanto riportato da Du \textit{et al.}~\cite{Du2010}.}


% ==========================================
% --- DISTRIBUZIONI E RELAZIONE MEDIA–SD
% ==========================================
\begin{figure}[H]
  \centering

  % ---------- Prima riga: istogrammi ----------
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/hist_normal_beta.pdf}
    \caption{Istogramma di $\beta$ nei campioni Normali}
  \end{subfigure}
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/hist_adjacent_beta.pdf}
    \caption{Istogramma di $\beta$ nei campioni Adiacenti}
  \end{subfigure}
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/hist_normal_m.pdf}
    \caption{Istogramma di $M$ nei campioni Normali}
  \end{subfigure}
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/hist_adjacent_m.pdf}
    \caption{Istogramma di $M$ nei campioni Adiacenti}
  \end{subfigure}

  \vspace{1em}

  % ---------- Seconda riga: relazioni media–SD ----------
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/msd_normal_beta.pdf}
    \caption{Relazione media–SD per $\beta$ nei Normali}
  \end{subfigure}
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/msd_adjacent_beta.pdf}
    \caption{Relazione media–SD per $\beta$ negli Adiacenti}
  \end{subfigure}
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/msd_normal_m.pdf}
    \caption{Relazione media–SD per $M$ nei Normali}
  \end{subfigure}
  \begin{subfigure}{0.24\textwidth}
    \includegraphics[width=\textwidth]{Figures/msd_adjacent_m.pdf}
    \caption{Relazione media–SD per $M$ negli Adiacenti}
  \end{subfigure}

  \caption{
  \textbf{Distribuzioni e relazioni media–deviazione standard dei valori di metilazione.}
  Gli istogrammi (prima riga) mostrano la tipica distribuzione a U dei $\beta$ e la maggiore simmetria degli $M$;  
  i pannelli inferiori evidenziano la forte dipendenza della varianza dalla media nei $\beta$, ridotta dopo la trasformazione logit in $M$.  
  Tutte le analisi sono state eseguite sulle 156{,}738 CpG rimaste dopo i filtri finora applicati.
  }
  \label{fig:hist_msd}
\end{figure}
