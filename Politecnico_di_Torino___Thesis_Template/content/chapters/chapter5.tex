...
proportion of near-constant probes.
...
\scriptsize
\caption{%
  Permutation-based overlap test for the three final $5{,}000$-CpG panels.
  For each cohort pair, the observed pairwise intersection size is compared
  against the empirical null distribution obtained by drawing $10{,}000$
  random selections of equal cardinality from the post-preprocessing
  platform intersection.
  Enrichment ratio is defined as the observed count divided by the
  null mean.
}
\label{tab:interfs_overlap}
\begin{tabular}{l c c c c}
\toprule
Pair & Observed & Null Mean & Enrichment & $p$-value \\
\midrule
GSE69914--GSE225845 & 148 & 69.0 & $\times 2.14$ & $< 10^{-4}$ \\
GSE69914--GSE287331 &  84 & 68.3 & $\times 1.23$ & $0.024$     \\
GSE225845--GSE287331 & 128 & 37.9 & $\times 3.38$ & $< 10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The observed values,
\[
J(\text{GSE69914},\text{GSE225845}) = 0.0150, \quad
J(\text{GSE69914},\text{GSE287331}) = 0.0085, \quad
...
This outcome is consistent with the broader pattern of weak single-locus
reproducibility documented in the pre-selection inter-dataset analysis
of Section~\ref{sec:inter_dataset_eda}, and is expected given the
substantial platform heterogeneity ($450$K vs.\ EPIC), cohort-specific
noise structure, and the HDLSS regime in which the selection operates.
...
\small
\caption{%
  Permutation-based overlap test for the three final $5{,}000$-CpG
  panels. For each cohort pair, the null distribution was obtained
  from $10{,}000$ random draws of two sets of $K{=}5{,}000$ CpGs
  from the post-preprocessing pairwise intersection
  $\mathcal{U}_{AB}$ (universe size reported).
  Enrichment ratio is defined as observed overlap divided by null
  mean; $p$-value is the proportion of null replications with
  overlap $\geq$ observed.
}
\label{tab:interfs_overlap}
\begin{tabular}{l r r r r r}
\toprule
Pair & $|\mathcal{U}_{AB}|$ & Observed & Null Mean
     & Enrichment & $p$-value \\
\midrule
GSE69914--GSE225845  & \texttt{[?]} & 148 & 69.0
    & $\times 2.14$ & $< 10^{-3}$ \\
GSE69914--GSE287331  & \texttt{[?]} &  84 & 68.3
    & $\times 1.23$ & $0.024$     \\
GSE225845--GSE287331 & \texttt{[?]} & 128 & 37.9
    & $\times 3.38$ & $< 10^{-3}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The two panels of Figure~\ref{fig:interfs_concordance} jointly
support a differentiated reading of cross-cohort consistency.
GSE69914 and GSE225845 capture a coherent and directionally
reproducible differential methylation structure, fulfilling the
conditions for the \emph{shared drift structure} regime
defined in Section~\ref{sec:inter_fs_synthesis}.
GSE287331, by contrast, identifies a distinct and internally robust
drift axis whose orientation is systematically inverted relative
to the other two cohorts, placing it in the \emph{directional
inversion} regime.
This asymmetry has direct consequences for cross-dataset
transferability, which is assessed quantitatively in the following subsection.
...
of Section~\ref{sec:inter_fs_synthesis}: the selected loci
...
\paragraph{GSE69914 $\leftrightarrow$ GSE225845.}
Bidirectional transfer between the two HM$450$/EPIC cohorts yields
AUC values of $0.776$ ($A{=}$GSE69914$\to B{=}$GSE225845) and
$0.794$ ($A{=}$GSE225845$\to B{=}$GSE69914), both substantially
above chance.
These results confirm that the shared drift axis identified through
concordance analysis translates into genuine discriminative
transferability: the $\Delta\beta$ vector estimated in one cohort
provides a meaningful ranking of Normal versus Adjacent samples in
the other, without any adaptation to the target distribution.
This constitutes the strongest cross-cohort evidence available in
this study, demonstrating that the GSE69914--GSE225845 pair captures
...
downstream modelling strategy developed in Chapter~\ref{chap:models}:
...
\scriptsize
\caption{%
  Sequential reduction of CpG dimensionality through the feature
  selection pipeline, applied to the training set of each cohort.
  Values at each stage reflect the number of CpG loci entering the
  subsequent step. Steps marked \textsuperscript{$\dagger$} produce
  a fixed-cardinality output by design.
  Intermediate values marked \texttt{[?]} are not recoverable from
  the current pipeline logs and should be filled from run outputs.
}
\label{tab:fs_reduction_steps_all}
\begin{tabular}{l r r r}
\toprule
Pipeline stage & GSE69914 & GSE225845 & GSE287331 \\
\midrule
Post-preprocessing space           & 251{,}483 & 530{,}683 & 486{,}725 \\
After Edgar variability filter ($\tau{=}0.05$, $\beta$-space)
                                   & 151{,}380 & 396{,}644 & 286{,}000 \\
After $M$-value SD trimming (bottom 2\%)
                                   & 148{,}352 & 388{,}711 & 280{,}280 \\
After stability + effect-size filter\textsuperscript{$\dagger$}
                                   & 15{,}000  & 15{,}000  & 15{,}000  \\
After correlation pruning ($|r|\geq 0.85$)
                                   & 6{,}300   & \texttt{[?]} & \texttt{[?]} \\
After constrained diversification\textsuperscript{$\dagger$}
                                   & 5{,}000   & 5{,}000   & 5{,}000   \\
\bottomrule
\end{tabular}
\end{table}
...
%----------------------------------------------------------------------
\subsection{Synthesis of Cross-Cohort Behaviour}
\label{sec:inter_fs_synthesis}
%----------------------------------------------------------------------

The multi-level evaluation conducted in Sections~\ref{sec:inter_fs_overlap}–-%
\ref{sec:inter_fs_transfer} reveals a pronounced and internally consistent
asymmetry in the cross-cohort behaviour of the three selected CpG panels.

At every analytical level — identity-level enrichment, signed and
magnitude-stratified effect concordance, and zero-shot discriminative
transferability — GSE69914 and GSE225845 exhibit mutually reinforcing
evidence of a shared epigenetic drift structure.
Their differential methylation profiles align in both polarity and
relative magnitude ordering, and the corresponding scoring functions
retain substantial discriminative power when applied to the partner
cohort without retraining (AUC $= 0.776$ and $0.794$, respectively).
The conjunction of significant set enrichment (permutation $p < 10^{-4}$),
near-complete directional concordance ($c = 0.980$), and bidirectional
transferability establishes this pair as satisfying all criteria for the
\emph{shared drift structure} regime.

GSE287331 occupies a qualitatively distinct position.
Despite yielding significant identity-level enrichment relative to the
permutation null — confirming the non-random nature of the selected
loci — both effect polarity and transfer performance indicate systematic
structural incompatibility with the other two cohorts.
Sign concordance collapses to $c = 0.125$--$0.357$, Spearman
correlations are negative ($\rho = -0.042$ to $-0.420$),
and zero-shot AUC values fall consistently in the range
$[0.017,\, 0.226]$, reflecting active inversion of the scoring
direction rather than mere absence of shared signal.
Magnitude-stratified analysis confirms that this inversion is
structural — not a low-amplitude artefact — and persists across
all deciles of effect size, including the strongest drift signals.
These results are consistent with the \emph{directional inversion}
regime: the panel selected from GSE287331 captures an internally
robust and biologically discriminative drift axis, but one whose
dominant orientation is systematically rotated relative to that
shared by GSE69914 and GSE225845.

Two non-exclusive mechanistic hypotheses may account for this behaviour.
First, platform-level effects: GSE287331 is the only cohort for which
BMIQ normalisation was found to suppress genuine group structure
(Section~\ref{sec:preproc_gse287331}), and its EPIC data carry
residual Type~I/II probe imbalance that may differentially distort
the estimated $\Delta\beta$ signs relative to the HM450-based
GSE69914.
Second, sampling design: GSE287331 was constructed along a
fine-grained tumour-proximity axis (TPxA), with Adjacent samples
operationally defined at a different tissue margin than in the other
two cohorts.
This design difference may capture a biologically distinct subtype of
epigenetic field effect, whose dominant methylation signature is
genuinely inverted relative to the broader Adjacent--Normal contrast
characterised in GSE69914 and GSE225845.
Disentangling these contributions would require platform-harmonised
reanalysis and is identified as a direction for future work.

A methodological remark on statistical significance is warranted.
Throughout this section, permutation $p$-values quantify enrichment
relative to the large-cardinality random baseline and should not be
conflated with biological equivalence.
Under the large-universe sampling regime operative here — where
$5{,}000$ loci are drawn from post-preprocessing intersections of
$\mathcal{O}(10^5)$ CpGs — even modest absolute overlaps can achieve
nominal significance.
Jaccard indices in the range $0.009$--$0.015$ demonstrate that
identity-level reproducibility remains structurally weak,
and that cross-cohort concordance must be interpreted primarily
through effect-structure and transferability evidence.

Collectively, the present analysis demonstrates that the epigenetic
drift characterising the Normal--Adjacent transition is \emph{partially}
replicable across independent cohorts, but that its orientation is
modulated by cohort-specific factors including platform architecture,
sampling design, and residual batch structure.
These structural asymmetries have direct implications for downstream
predictive modelling: naive cross-cohort transfer of linear
$\Delta\beta$ signatures is feasible between GSE69914 and GSE225845
but actively misleading when GSE287331 is involved, motivating
orientation-aware or cohort-adaptive representational strategies
investigated in Chapter~\ref{chap:models}.
